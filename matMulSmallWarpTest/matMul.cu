...
__global__ void MatMulKernel(Matrix A, Matrix B, Matrix C, int divider) {
    float Cvalue = 0.0;
    int row = (blockIdx.y * blockDim.y + threadIdx.y) * divider;
    int col = (blockIdx.x * blockDim.x + threadIdx.x) * divider;

    if(row >= A.height || col >= B.width) return;

    for(int row_iter = row; row_iter < row + divider; row_iter++)
    {
        for(int col_iter = col; col_iter < col + divider; col_iter++)
        {
            Cvalue = 0.0;
            for (int e = 0; e < A.width; ++e)
                Cvalue += (A.elements[row_iter * A.width + e]) * (B.elements[e * B.width + col_iter]);
            C.elements[row_iter * C.width + col_iter] = Cvalue;
        }
    }
}

__global__ void MatMulKernel_half(Matrix_half A, Matrix_half B, Matrix_half C, int divider) {
    ...
    for(int row_iter = row; row_iter < row + divider; row_iter++)
    {
        for(int col_iter = col; col_iter < col + divider; col_iter++)
        {
            Cvalue = __float2half(0.0);
            for (int e = 0; e < A.width; ++e)
            {
                Cvalue = __hadd( Cvalue, 
                                 __hmul( (A.half_elements[row_iter * A.width + e]), 
                                         (B.half_elements[e * B.width + col_iter]) ) 
                        );
            }
            C.half_elements[row_iter * C.width + col_iter] = Cvalue;
        }
    }
}
...
void MatMul_half(const Matrix A, const Matrix B, Matrix C, int divider) {
    ...
    // Invoke kernel
    dim3 dimBlock(BLOCK_SIZE / divider, BLOCK_SIZE / divider);
    dim3 dimGrid((B.width + dimBlock.x - 1) / dimBlock.x,
            (A.height + dimBlock.y - 1) / dimBlock.y);

    long t = timer_get();
    MatMulKernel_half<<<dimGrid, dimBlock>>>(d_A, d_B, d_C, divider);
...
void MatMul(const Matrix A, const Matrix B, Matrix C, int divider) {
    Matrix d_A;
    d_A.width = A.width;
    d_A.height = A.height;
    ...
    long t = timer_get();
    MatMulKernel<<<dimGrid, dimBlock>>>(d_A, d_B, d_C, divider);
    err = cudaDeviceSynchronize();
...
int main(int argc, char* argv[]){
    ...
    */
    ...
    if(strcmp(argv[3], "normal") == 0)
        MatMul(A,B,C, divider);
    else if(strcmp(argv[3], "half") == 0)
        MatMul_half(A, B, C, divider);
    else{
    ...
    for(int i = 0; i < min(64, A.height); i++){
        for(int j = 0; j < min(64, A.width); j++)
            printf("%d ", (int)A.elements[i*A.width + j]);
        printf("\n");
    }
    printf("\n");

    for(int i = 0; i < min(64, B.height); i++){
        for(int j = 0; j < min(64, B.width); j++)
            printf("%d ", (int)B.elements[i*B.width + j]);
        printf("\n");
    }
    printf("\n");

    for(int i = 0; i < min(64, C.height); i++){
        for(int j = 0; j < min(64, C.width); j++)
